...
using reflective event-chain Monte Carlo.
...
velocity update in a disk collision reflects the velocity by the line connecting the two involved disks, and the active
index is transferred to the collision disk. The active disk and its velocity are resampled after each sampling.
...
for sample in range(n_samples):
    ...
    while True:
        ...
        if min(wall_times) < min(pair_times):
            ...
            dot = e_parallel[0] * vel[0] + e_parallel[1] * vel[1]
            vel = [-vel[0] + 2.0 * e_parallel[0] * dot, -vel[1] + 2.0 * e_parallel[1] * dot]
            abs_vel = math.sqrt(vel[0] ** 2 + vel[1] ** 2)
            vel = [v / abs_vel for v in vel]
            active = target
